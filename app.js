...
const ROLLING_IDS=new Set([
  ...
  'portfolioDaily','dailyEstimate','reward1d','reward1w','reward1m','reward1y',
...
function renderMarketPrice(){
  ...
  if(change){
    const direction=state.change>0?'positive':state.change<0?'negative':'neutral';
    const text=`${state.change>0?'+':''}${state.change.toFixed(2)}% nelle 24h`;
    if(change.textContent!==text) change.textContent=text;
    change.className=`inj-live-change fixed-percentage ${direction}`;
...
function drawChart(canvas, values, positive=true, hover=null){
  ...
  const {visible:data}=getChartSlice(canvas.id,values);
  if(!data.length) return;

  let min=Math.min(...data.map(x=>x.v)),max=Math.max(...data.map(x=>x.v));
  if(max===min){max+=1;min-=1}
  const pad=14;
  const points=data.map((row,i)=>({
    x:pad+(data.length===1?.5:i/(data.length-1))*(w-pad*2),
    y:pad+(max-row.v)/(max-min)*(h-pad*2),
    ...row
  }));

  const styles=getComputedStyle(document.documentElement);
  const accent=styles.getPropertyValue(positive?'--accent':'--red').trim()||(positive?'#22d3a6':'#ff5d6c');
  const panel=getComputedStyle(document.body).getPropertyValue('--panel').trim()||'#030303';
  const light=document.body.classList.contains('light');
  const motion=!document.body.classList.contains('motion-off') &&
  ...
  ctx.save();
  ctx.lineWidth=1;
  ctx.strokeStyle=light?'rgba(15,23,42,.075)':'rgba(255,255,255,.055)';
  ctx.setLineDash([2,6]);
  for(let row=1;row<=3;row++){
    const y=pad+(h-pad*2)*(row/4);
  ...
  if(points.length>1){
    ...
    gradient.addColorStop(.55,accent+'14');
    ...
    gradient.addColorStop(1,accent+'00');
    ctx.beginPath();ctx.moveTo(pad,h);ctx.lineTo(pad,p.y);ctx.lineTo(w-pad,p.y);ctx.lineTo(w-pad,h);ctx.closePath();
    ctx.fillStyle=gradient;ctx.fill();
    ctx.beginPath();ctx.moveTo(pad,p.y);ctx.lineTo(w-pad,p.y);
    ctx.strokeStyle=accent;ctx.lineWidth=2.15;ctx.lineCap='round';ctx.stroke();
  }

  // Pulsing LIVE marker on the newest value.
  const livePoint=points.at(-1);
  if(livePoint){
    const wave=motion?(Math.sin(Date.now()/330)+1)/2:.42;
    ctx.save();
    ctx.beginPath();
    ctx.arc(livePoint.x,livePoint.y,7.5+wave*3.5,0,Math.PI*2);
    ctx.fillStyle=accent;
    ctx.globalAlpha=.07+wave*.08;
    ctx.fill();
    ctx.globalAlpha=1;
    ctx.beginPath();ctx.arc(livePoint.x,livePoint.y,3.35,0,Math.PI*2);
  ...
  if(hover&&Number.isInteger(hover.index)&&points[hover.index]){
    ...
    ctx.save();
    ctx.setLineDash([3,5]);
    ctx.strokeStyle=light?'rgba(15,23,42,.30)':'rgba(255,255,255,.28)';
    ctx.lineWidth=.8;
...
function startStartupLoader(){
  ...
  const timer=setInterval(()=>{
    ...
    if(elapsed>=maxDuration){stop();finishStartupLoader();status('offline','Modalità offline pronta');}
  },150);
...
(() => {
  ...
  function openDb(){
    return new Promise(resolve=>{
      ...
      let req;
      try{ req=indexedDB.open('inj-terminal-v5',1); }catch{ resolve(null); return; }
...
(() => {
  ...
  function pulseValue(el,oldText,newText){
    if(document.documentElement.dataset.motionEngine==='v82') return;
...
(()=>{
  ...
  function renderRewardForecast(){
    const compound=forecastMode==='compound';
    const formatUsd=value=>new Intl.NumberFormat('en-US',{
      style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:2
    }).format(Math.max(0,num(value)));
    const set=(id,usdId,value)=>{
      const injValue=num(value);
      const injEl=document.getElementById(id);
      const usdEl=document.getElementById(usdId);
      if(injEl) injEl.textContent=injValue>0?`${injValue.toFixed(injValue<1?6:4)} INJ`:'—';
      if(usdEl) usdEl.textContent=injValue>0&&num(state.price)>0?formatUsd(injValue*num(state.price)):'—';
    };
    const today=projectedReward(1,compound);
    const tomorrow=projectedReward(2,compound)-today;
    set('forecastToday','forecastTodayUsd',today);
    set('forecastTomorrow','forecastTomorrowUsd',tomorrow);
    set('forecast7d','forecast7dUsd',projectedReward(7,compound));
    set('forecast30d','forecast30dUsd',projectedReward(30,compound));
    set('forecast365d','forecast365dUsd',projectedReward(365,compound));
...
(()=>{
  ...
  let motionEnabled=storage.get('inj_motion_v72','on')!=='off';
  ...
  modeButtons.data?.addEventListener('click',()=>{
    ...
    storage.set('inj_data_mode_v56',active?'on':'off');
  ...
  modeButtons.theme?.addEventListener('click',()=>{
    ...
    storage.set('inj_theme',active?'light':'dark');
    syncInterfaceModes(); requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')));
  });
  modeButtons.motion?.addEventListener('click',()=>{
    motionEnabled=!motionEnabled;
    document.body.classList.toggle('reduce-motion',!motionEnabled);
    storage.set('inj_motion_v72',motionEnabled?'on':'off');
    syncInterfaceModes();
  });
  if(storage.get('inj_data_mode_v56')==='on')document.body.classList.add('data-mode');
  if(storage.get('inj_theme')==='light')document.body.classList.add('light');
  document.body.classList.toggle('reduce-motion',!motionEnabled);
  syncInterfaceModes();
})();
...
(() => {
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    initLiveDisplay();
    lockAllPercentages();
    // Live Display is updated by the single v8 controller below. Keeping this
    // legacy updater disabled prevents 4/6-decimal formats from alternating.
...
(() => {
  ...
  function applyTheme(theme){
    ...
    set('inj_theme',theme==='light'?'light':'dark');
    const themeColor=document.querySelector('meta[name="theme-color"]');
    if(themeColor) themeColor.setAttribute('content',theme==='light'?'#f3f5f7':theme==='blue'?'#07111f':'#000000');
...
/* v8.2 — definitive rolling-value engine + rear card pulse */
(() => {
  document.documentElement.dataset.motionEngine='v82';
  ...
  function parseDisplayNumber(value){
    ...
    if(comma>=0 && dot>=0){
      ...
      token=decimals>0 && decimals<=8 ? token.replace(',','.') : token.replace(/,/g,'');
    }else if(dot>=0){
      const decimals=token.length-dot-1;
      if(decimals===3 && (token.match(/\./g)||[]).length>1) token=token.replace(/\./g,'');
    }
    const n=Number(token);
    return Number.isFinite(n)?n:NaN;
  }

  numericFromText = parseDisplayNumber;
  ...
  function stableChars(el,text){
    el.replaceChildren();
    el.classList.add('rolling-number');
    for(const char of String(text)){
      const node=document.createElement('span');
      node.className=/\d/.test(char)?'roll-char roll-digit':'roll-char roll-symbol';
      node.textContent=char;
      el.appendChild(node);
    }
  }

  function directionClass(el,direction){
    el.classList.remove('value-direction-up','value-direction-down');
    void el.offsetWidth;
    el.classList.add(`value-direction-${direction}`);
    clearTimeout(el._directionTimer);
    el._directionTimer=setTimeout(()=>{
      el.classList.remove('value-direction-up','value-direction-down');
    },ANIMATION_MS+260);
  }

  function pulseHost(el,direction){
    const host=el.closest('#dashboardView .card');
    if(!host) return;
    const now=performance.now();
    const last=Number(host.dataset.lastPulseAt||0);
    const previousDirection=host.dataset.pulseDirection;
    if(now-last<PULSE_THROTTLE_MS && previousDirection===direction) return;
    host.dataset.lastPulseAt=String(now);
    host.dataset.pulseDirection=direction;
    host.classList.remove('live-data-up','live-data-down','value-glow-up','value-glow-down','data-pulse-up','data-pulse-down');
    void host.offsetWidth;
    host.classList.add(`data-pulse-${direction}`);
    clearTimeout(host._dataPulseTimer);
    host._dataPulseTimer=setTimeout(()=>{
      host.classList.remove('data-pulse-up','data-pulse-down');
    },PULSE_MS+80);
  }

  signalDashboardCard = function(id,numericValue){
    const el=document.getElementById(id);
    const next=Number(numericValue);
    ...
    pulseHost(el,next>previous?'up':'down');
  };

  function animateWholeValue(el,previousText,nextText,direction,token){
    const stack=document.createElement('span');
    stack.className=`roll-whole-stack roll-whole-${direction}`;
    const oldNode=document.createElement('span');
    oldNode.className='roll-whole-old';
    oldNode.textContent=previousText;
    const newNode=document.createElement('span');
    newNode.className='roll-whole-new';
    newNode.textContent=nextText;
    stack.append(oldNode,newNode);
    el.replaceChildren(stack);
    setTimeout(()=>{
      if(el.dataset.rollToken!==token) return;
      stableChars(el,nextText);
    },ANIMATION_MS+30);
  }
  ...
  rollValue = function(id,formatted,numericValue){
    ...
    const previousText=el.dataset.rollText ?? el.textContent ?? '';
    const previousValue=Number(el.dataset.rollValue);
    const explicit=Number(numericValue);
    const nextValue=Number.isFinite(explicit)?explicit:parseDisplayNumber(nextText);
    ...
    const comparable=previousText && !previousText.includes('—') &&
      Number.isFinite(previousValue) && Number.isFinite(nextValue) && previousValue!==nextValue;
    const direction=comparable && nextValue<previousValue?'down':'up';
    const token=String((Number(el.dataset.rollToken)||0)+1);
    ...
    if(!comparable || !motionEnabled() || stableLiveIds.has(id)){
      stableChars(el,nextText);
      if(comparable){
        directionClass(el,direction);
        signalDashboardCard(id,nextValue);
      }else if(Number.isFinite(nextValue)){
        el.dataset.frameValue=String(nextValue);
      }
      return;
    }
    ...
    // Per-digit rolling when the formatted layout is stable; whole-value rolling
    // when currency/locale formatting changes the string length.
    if(previousText.length!==nextText.length){
      animateWholeValue(el,previousText,nextText,direction,token);
      return;
    }
    ...
    for(let i=0;i<nextText.length;i++){
      ...
      const digit=/\d/.test(newChar);
      const changed=digit && /\d/.test(oldChar) && oldChar!==newChar;
      if(changed){
        changedDigits++;
        const slot=document.createElement('span');
        slot.className=`roll-slot roll-${direction}`;
        const oldNode=document.createElement('span');
        oldNode.className='roll-old'; oldNode.textContent=oldChar;
        const newNode=document.createElement('span');
        newNode.className='roll-new'; newNode.textContent=newChar;
        slot.append(oldNode,newNode);
        el.appendChild(slot);
      }else{
        const node=document.createElement('span');
        node.className=digit?'roll-char roll-digit':'roll-char roll-symbol';
  ...
  // Include every currently numeric dashboard output in the rolling engine.
  [
    'stakeAllocation','stakeAllocationText','liquidAllocationText','athValue',
    'milestoneRemaining','milestoneGoal','milestonePercent','requiredInjPrice',
    'rewardTargetStaked','rewardTargetDailyInj','rewardTargetApr','forecastToday',
    'forecastTomorrow','forecast7d','forecast30d','forecast365d','walletStakedShare',
    'simulatorSupplyUsed','networkTotalStaked','networkStakedPct','networkBurnedPct',
    'healthScore','apiLatency','wsMonitor','refreshMonitor','proHealthScore'
  ].forEach(id=>ROLLING_IDS.add(id));

  document.addEventListener('DOMContentLoaded',()=>{
    document.querySelectorAll('#dashboardView .card').forEach(card=>{
      card.classList.remove('live-data-up','live-data-down','value-glow-up','value-glow-down');
    });
  },{once:true});
})();


/* v8.3 — rock-solid Live Display + pulse only on visible data changes */
(() => {
  document.documentElement.dataset.motionEngine='v83';

  const LIVE_IDS = new Set([
    'landscapeMarketPrice','landscapeWalletWorth','landscapeWalletInj',
    'landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards'
  ]);
  const animatedRollValue = rollValue;

  function finite(value){
    const n=Number(value);
    return Number.isFinite(n)?n:0;
  }
  function canonicalLiveText(id, fallback){
    const price=finite(state.price);
    const available=finite(state.available);
    const staked=finite(state.staked);
    const rewards=finite(state.rewards);
    const total=available+staked+rewards;
    const inj6=value=>finite(value).toLocaleString('it-IT',{
      minimumFractionDigits:6,maximumFractionDigits:6
    });
    switch(id){
      case 'landscapeMarketPrice': return displayMoney(price,4);
      case 'landscapeWalletWorth': return displayMoney(total*price,2);
      case 'landscapeWalletInj': return `${inj6(total)} INJ`;
      case 'landscapeWalletAvailable': return inj6(available);
      case 'landscapeWalletStaked': return inj6(staked);
      case 'landscapeWalletRewards': return inj6(rewards);
      default: return String(fallback);
    }
  }

  // Live Display never rolls, flashes or reconstructs digit slots. It changes
  // text only when the final visible string is genuinely different.
  rollValue = function(id, formatted, numericValue){
    if(!LIVE_IDS.has(id)) return animatedRollValue(id,formatted,numericValue);
    const el=document.getElementById(id);
    if(!el) return;
    const nextText=canonicalLiveText(id,formatted);
    const nextValue=Number(numericValue);
    if(el.dataset.liveStableText===nextText && el.textContent===nextText){
      if(Number.isFinite(nextValue)) el.dataset.rollValue=String(nextValue);
      return;
    }
    el.classList.remove(
      'rolling-number','fixed-percentage','positive','negative','neutral',
      'value-direction-up','value-direction-down','value-tick-up','value-tick-down'
    );
    el.style.removeProperty('min-inline-size');
    el.replaceChildren(document.createTextNode(nextText));
    el.dataset.liveStableText=nextText;
    el.dataset.rollText=nextText;
    if(Number.isFinite(nextValue)) el.dataset.rollValue=String(nextValue);
  };

  // A card pulse is keyed to the visible value, not to repeated renders or
  // invisible floating-point noise. First paint only records the baseline.
  signalDashboardCard = function(id,numericValue){
    const el=document.getElementById(id);
    const next=Number(numericValue);
    const host=el?.closest('#dashboardView .card');
    if(!el || !host || !Number.isFinite(next)) return;

    const visible=String(el.dataset.rollText ?? el.textContent ?? '').trim();
    const key=`${visible}|${next}`;
    const previousKey=el.dataset.frameVisibleKey;
    const previous=Number(el.dataset.frameValue);
    el.dataset.frameVisibleKey=key;
    el.dataset.frameValue=String(next);

    if(previousKey===undefined || !Number.isFinite(previous)) return;
    if(previousKey===key || previous===next) return;

    const direction=next>previous?'up':'down';
    const now=performance.now();
    const lastKey=host.dataset.lastVisiblePulseKey;
    const lastAt=Number(host.dataset.lastVisiblePulseAt||0);
    const pulseKey=`${id}:${key}`;
    if(lastKey===pulseKey && now-lastAt<1100) return;

    host.dataset.lastVisiblePulseKey=pulseKey;
    host.dataset.lastVisiblePulseAt=String(now);
    host.classList.remove(
      'live-data-up','live-data-down','value-glow-up','value-glow-down',
      'data-pulse-up','data-pulse-down'
    );
    void host.offsetWidth;
    host.classList.add(`data-pulse-${direction}`);
    clearTimeout(host._dataPulseTimer);
  ...
  function stabiliseLivePanel(){
    const panel=document.getElementById('landscapeLive');
    if(!panel) return;
    panel.querySelectorAll('.value-glow-up,.value-glow-down,.data-pulse-up,.data-pulse-down').forEach(node=>{
      node.classList.remove('value-glow-up','value-glow-down','data-pulse-up','data-pulse-down');
    });
    LIVE_IDS.forEach(id=>{
      const el=document.getElementById(id);
      if(el) rollValue(id,canonicalLiveText(id,el.textContent),Number(el.dataset.rollValue));
    });
  }

  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',stabiliseLivePanel,{once:true});
  }else stabiliseLivePanel();
})();

/* v8.4 — layout-locked numbers + visible-change-only pulse */
(() => {
  document.documentElement.dataset.motionEngine = 'v84';
  ...
  function canonicalLiveText(id, fallback){
    const price = finite(state.price) || 0;
    const available = finite(state.available) || 0;
    const staked = finite(state.staked) || 0;
    const rewards = finite(state.rewards) || 0;
    const total = available + staked + rewards;
    const inj6 = value => Number(value || 0).toLocaleString('it-IT', {
  ...
  function measureInEm(el, text){
    const style = getComputedStyle(el);
    const size = parseFloat(style.fontSize) || 16;
    if(!context) return Math.max(1, String(text).length * .64);
    context.font = [
      style.fontStyle,
      style.fontVariant,
      style.fontWeight,
    ...
    return Math.max(1, (width + 2) / size);
  }

  function lockValueBox(el, ...texts){
    let reserve = Number(el.dataset.valueReserveEm || 0);
    for(const text of texts){
      if(text !== undefined && text !== null) reserve = Math.max(reserve, measureInEm(el, String(text)));
    }
    reserve = Math.ceil(reserve * 100) / 100;
    el.dataset.valueReserveEm = String(reserve);
    el.style.setProperty('--stable-value-width', `${reserve}em`);
    el.classList.add('stable-value-box');
  }
  ...
  function stableChars(el, text){
    ...
    el.classList.add('rolling-number','stable-value-box');
  }

  function setDirection(el, direction){
    el.classList.remove('value-direction-up','value-direction-down');
    if(!motionEnabled()) return;
    el.classList.add(`value-direction-${direction}`);
    clearTimeout(el._v84DirectionTimer);
    el._v84DirectionTimer = setTimeout(() => {
      el.classList.remove('value-direction-up','value-direction-down');
    }, ANIMATION_MS + 180);
  }

  function triggerPulse(host, direction){
    ...
    void host.offsetWidth;
    host.classList.add(`data-pulse-${direction}`);
    clearTimeout(host._v84PulseTimer);
    host._v84PulseTimer = setTimeout(() => {
      host.classList.remove('data-pulse-up','data-pulse-down');
    }, PULSE_MS + 80);
  }

  /* Compares only the final text users can see. Floating-point noise and
     repeated renders can update the baseline, but can never fire the glow. */
  signalDashboardCard = function(id, numericValue){
    const el = document.getElementById(id);
    const host = el?.closest('#dashboardView .card');
    const next = finite(numericValue);
    if(!el || !host || !Number.isFinite(next)) return;

    /* The calculator remains live, but never triggers card-level pulse/glow. */
    if(el.closest('.price-simulator-card')){
      el.dataset.pulseVisibleText = String(el.textContent ?? '').trim();
      el.dataset.pulseNumericValue = String(next);
      return;
    }

    const visible = String(el.dataset.rollText ?? el.textContent ?? '').trim();
    const previousVisible = el.dataset.pulseVisibleText;
    const previousNumber = finite(el.dataset.pulseNumericValue);
    el.dataset.pulseVisibleText = visible;
    el.dataset.pulseNumericValue = String(next);

    if(previousVisible === undefined || !Number.isFinite(previousNumber)) return;
    if(previousVisible === visible) return;
    triggerPulse(host, next < previousNumber ? 'down' : 'up');
  };
  ...
  rollValue = function(id, formatted, numericValue){
    ...
    const nextText = LIVE_IDS.has(id)
      ? canonicalLiveText(id, formatted)
      : String(formatted ?? '');
    const previousText = String(el.dataset.rollText ?? el.textContent ?? '');
    const previousValue = finite(el.dataset.rollValue);
    const explicitValue = finite(numericValue);
    const nextValue = Number.isFinite(explicitValue)
      ? explicitValue
      : finite(typeof numericFromText === 'function' ? numericFromText(nextText) : NaN);

    lockValueBox(el, previousText, nextText);

    if(previousText === nextText){
      el.dataset.rollText = nextText;
      if(Number.isFinite(nextValue)) el.dataset.rollValue = String(nextValue);
      if(Number.isFinite(nextValue)) signalDashboardCard(id, nextValue);
      return;
    }
    ...
    if(Number.isFinite(nextValue)) el.dataset.rollValue = String(nextValue);
    el.classList.remove(
      'fixed-percentage','positive','negative','neutral',
      'value-tick-up','value-tick-down'
    );

    /* Live Display is deliberately static. A length change is also painted
       statically: rolling the whole string is what made cards appear to jump. */
    if(LIVE_IDS.has(id) || !comparable || !motionEnabled() || previousText.length !== nextText.length){
      stableChars(el, nextText);
      if(comparable) setDirection(el, direction);
      if(Number.isFinite(nextValue)) signalDashboardCard(id, nextValue);
      return;
    }

    const fragment = document.createDocumentFragment();
    let animatedDigits = 0;
    for(let i = 0; i < nextText.length; i++){
      const oldChar = previousText[i] ?? '';
      const newChar = nextText[i];
      const isDigit = /\d/.test(newChar);
      const changed = isDigit && /\d/.test(oldChar) && oldChar !== newChar;
      if(changed){
        animatedDigits++;
        const slot = document.createElement('span');
        slot.className = `roll-slot roll-${direction}`;
        const oldNode = document.createElement('span');
        oldNode.className = 'roll-old';
        oldNode.textContent = oldChar;
        const newNode = document.createElement('span');
        newNode.className = 'roll-new';
        newNode.textContent = newChar;
        slot.append(oldNode, newNode);
        fragment.appendChild(slot);
      }else{
        const node = document.createElement('span');
        node.className = isDigit ? 'roll-char roll-digit' : 'roll-char roll-symbol';
        node.textContent = newChar;
        fragment.appendChild(node);
      }
    }
    ...
    const token = String((Number(el.dataset.v84Token) || 0) + 1);
    el.dataset.v84Token = token;
    setTimeout(() => {
      if(el.dataset.v84Token !== token) return;
      stableChars(el, nextText);
    }, ANIMATION_MS + 30);
  };

  function stabiliseExistingValues(){
    const ids = typeof ROLLING_IDS !== 'undefined' ? [...ROLLING_IDS] : [];
    for(const id of ids){
      const el = document.getElementById(id);
      if(!el) continue;
      const text = String(el.dataset.rollText ?? el.textContent ?? '');
      lockValueBox(el, text);
    }
    for(const id of LIVE_IDS){
      const el = document.getElementById(id);
      if(!el) continue;
      const text = canonicalLiveText(id, el.textContent);
      el.dataset.rollText = text;
      stableChars(el, text);
      lockValueBox(el, text);
    }
  ...
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', stabiliseExistingValues, {once:true});
  }else{
    stabiliseExistingValues();
  }
})();


/* v8.5 — remove accumulated width locks and enforce semantic performance colours */
(() => {
  function clearWidthLocks(root=document){
    root.querySelectorAll('.stable-value-box,[data-value-reserve-em]').forEach(el=>{
      el.style.removeProperty('--stable-value-width');
      el.style.removeProperty('width');
      el.style.removeProperty('inline-size');
      delete el.dataset.valueReserveEm;
    });
  }

  function applyDirectionClass(el, value){
    if(!el) return;
    const n=Number(value);
    el.classList.remove('up','down','neutral','performance-positive','performance-negative');
    if(!Number.isFinite(n) || n===0){
      el.classList.add('neutral');
    }else if(n>0){
      el.classList.add('up','performance-positive');
    }else{
      el.classList.add('down','performance-negative');
    }
  }

  function paintPricePerformance(){
    const change=Number(state?.change);
    ['marketChange','priceChange','landscapeMarketChange'].forEach(id=>applyDirectionClass(document.getElementById(id),change));
  }

  const previousRollValue=rollValue;
  rollValue=function(id,formatted,numericValue){
    const result=previousRollValue.call(this,id,formatted,numericValue);
    const el=document.getElementById(id);
    if(el){
      el.style.removeProperty('--stable-value-width');
      el.style.removeProperty('width');
    ...
    if(id==='marketChange'||id==='priceChange'||id==='landscapeMarketChange'){
      applyDirectionClass(el,numericValue);
    }
    return result;
  };

  const previousRenderMarketPrice=typeof renderMarketPrice==='function'?renderMarketPrice:null;
  if(previousRenderMarketPrice){
    renderMarketPrice=function(...args){
      const result=previousRenderMarketPrice.apply(this,args);
      paintPricePerformance();
      return result;
    };
  }

  const previousSetPerformance=typeof setPerformance==='function'?setPerformance:null;
  if(previousSetPerformance){
    setPerformance=function(id,change){
      previousSetPerformance(id,change);
      const value=change&&Number.isFinite(Number(change.pct))?Number(change.pct):change&&Number(change.usd);
      applyDirectionClass(document.getElementById(id),value);
    };
  }
    ...
    const observer=new MutationObserver(mutations=>{
      for(const mutation of mutations){
        if(mutation.type==='childList') mutation.addedNodes.forEach(node=>{
          if(node.nodeType===1) clearWidthLocks(node);
        });
      }
    });
...
/* v8.7 — Live Display per-digit rolling + directional colour */
(() => {
  const LIVE_IDS = new Set([
    'landscapeMarketPrice','landscapeWalletWorth','landscapeWalletInj',
    'landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards'
  ...
  const RESET_MS = 640;
  const inheritedRollValue = rollValue;
  ...
  function paintStatic(el, text){
    const fragment = document.createDocumentFragment();
    for(const char of String(text)) fragment.appendChild(createStaticChar(char));
    el.replaceChildren(fragment);
    el.classList.add('rolling-number');
  }

  rollValue = function(id, formatted, numericValue){
    if(!LIVE_IDS.has(id)) return inheritedRollValue.call(this,id,formatted,numericValue);

    const el = document.getElementById(id);
    if(!el) return;

    const nextText = String(formatted ?? '');
    const previousText = String(el.dataset.liveAnimatedText ?? el.dataset.rollText ?? el.textContent ?? '');
    const previousValue = finite(el.dataset.liveAnimatedValue ?? el.dataset.rollValue);
    const nextValue = finite(numericValue);

    el.dataset.liveAnimatedText = nextText;
    el.dataset.rollText = nextText;
    if(Number.isFinite(nextValue)){
      el.dataset.liveAnimatedValue = String(nextValue);
      el.dataset.rollValue = String(nextValue);
    }

    if(previousText === nextText) return;

    const comparable = previousText.length === nextText.length &&
      Number.isFinite(previousValue) && Number.isFinite(nextValue) &&
      previousValue !== nextValue;

    if(!comparable || !motionEnabled()){
      paintStatic(el,nextText);
      return;
    }
    ...
    for(let i=0;i<nextText.length;i++){
      const oldChar = previousText[i] ?? '';
      const newChar = nextText[i];
      const changedDigit = /\d/.test(oldChar) && /\d/.test(newChar) && oldChar !== newChar;
      const node = createStaticChar(newChar);
      if(changedDigit){
        node.classList.add(direction === 'up' ? 'digit-change-up' : 'digit-change-down');
        changedDigits++;
      }
      fragment.appendChild(node);
    }
    ...
    if(!changedDigits) return;

    clearTimeout(el._liveDigitResetTimer);
    const token = String((Number(el.dataset.liveDigitToken)||0)+1);
    el.dataset.liveDigitToken = token;
    el._liveDigitResetTimer = setTimeout(() => {
      if(el.dataset.liveDigitToken !== token) return;
      paintStatic(el,nextText);
    },RESET_MS);
  };
})();

/* v9.8 — automatically fit long dashboard values to their real card width */
(() => {
  const MOBILE_MAX = Number.POSITIVE_INFINITY;
  const TARGETS = [
    '#dashboardView .metric-grid .metric-card > strong',
    '#dashboardView #marketPrice',
    '#dashboardView .networth-live-card .hero-value',
    '#dashboardView .reward-speed-card > .focus-value',
    '#dashboardView .ath-card > .focus-value',
    '#dashboardView .sentiment-card > .focus-value',
    '#dashboardView .milestone-values > strong',
    '#dashboardView .reward-forecast-grid > div > strong',
    '#dashboardView .monitor-grid > div > strong',
    '#dashboardView .simulator-comparison-grid > div > strong',
    '#dashboardView .network-share-item > strong',
    '#dashboardView .tokenomics-stat > strong',
    '#dashboardView .simulator-wallet-value > div > strong',
    '#dashboardView .price-simulator-result > strong',
    '#dashboardView .reward-target-result > strong',
    '#dashboardView .simulator-live-cap > strong',
    '#dashboardView .projection-item > strong'
  ].join(',');
  ...
  function minFontSize(el){
    if(el.id === 'marketPrice' || el.id === 'netWorthUsd') return 20;
    if(el.id === 'rewardPerSecond') return 9;
    if(el.classList.contains('focus-value')) return 12;
    return 12;
  }

  function restore(el){
    if(!Object.prototype.hasOwnProperty.call(el.dataset,'mobileFitOriginalFont')) return;
    const original = el.dataset.mobileFitOriginalFont;
    if(original) el.style.setProperty('font-size',original,'important');
    else el.style.removeProperty('font-size');
    delete el.dataset.mobileFitOriginalFont;
    el.classList.remove('mobile-value-fitted');
  }
  ...
  function fitElement(el){
    ...
    if(!Object.prototype.hasOwnProperty.call(el.dataset,'mobileFitOriginalFont')){
      el.dataset.mobileFitOriginalFont = el.style.fontSize || '';
    }
    ...
    let current = parseFloat(computed.fontSize) || 16;
    const minimum = minFontSize(el);

    /* Three passes are enough even for rolling-digit wrappers. */
    for(let pass=0; pass<3; pass++){
      const required = Math.ceil(el.scrollWidth);
      if(required <= available + 1) break;
      const ratio = Math.max(.55, (available / required) * .975);
      const next = Math.max(minimum, current * ratio);
      if(Math.abs(next-current) < .15) break;
      current = next;
      el.style.setProperty('font-size',`${current.toFixed(2)}px`,'important');
      computed = getComputedStyle(el);
    }

    /* Final safety pass: reduce in small steps until every digit is visible. */
    let guard = 0;
    while(el.scrollWidth > available + 1 && current > minimum && guard < 14){
      current = Math.max(minimum,current-.5);
      el.style.setProperty('font-size',`${current.toFixed(2)}px`,'important');
      guard++;
    }

    el.classList.toggle('mobile-value-fitted', current < (parseFloat(computed.fontSize) || current));
  }

  function fitAll(){
    raf = 0;
    document.querySelectorAll(TARGETS).forEach(fitElement);
  }
  ...
  function init(){
    ...
    mutationObserver.observe(dashboard,{subtree:true,childList:true,characterData:true});

    if('ResizeObserver' in window){
      resizeObserver = new ResizeObserver(scheduleFit);
      resizeObserver.observe(dashboard);
      document.querySelectorAll(TARGETS).forEach(el=>resizeObserver.observe(el.parentElement || el));
    }
...
/* v9.9 — equal mobile chart geometry and stable redraw */
(() => {
  let timer = 0;
  const redrawEqualCharts = () => {
    clearTimeout(timer);
    const run = () => { try { if (typeof drawAll === 'function') drawAll(); } catch {} };
    requestAnimationFrame(() => requestAnimationFrame(run));
    timer = setTimeout(run, 180);
  };
  window.addEventListener('resize', redrawEqualCharts, { passive: true });
  window.addEventListener('orientationchange', redrawEqualCharts, { passive: true });
  window.visualViewport?.addEventListener('resize', redrawEqualCharts, { passive: true });
  document.addEventListener('visibilitychange', () => { if (!document.hidden) redrawEqualCharts(); });
  const bodyObserver = new MutationObserver(mutations => {
    if (mutations.some(m => m.type === 'attributes' && m.attributeName === 'class')) redrawEqualCharts();
  });
  const start = () => {
...
/* v10.4.1 — Live Display vertical rolling without rebound */
(() => {
  const LIVE_IDS = new Set([
    'landscapeMarketPrice','landscapeWalletWorth','landscapeWalletInj',
    'landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards'
  ...
  function staticCharacter(character){
    const node = document.createElement('span');
    node.className = /\d/.test(character)
      ? 'roll-char roll-digit'
      : 'roll-char roll-symbol';
  ...
  function prepareElement(element){
    element.classList.remove(
      'fixed-percentage','positive','negative','neutral',
      'value-direction-up','value-direction-down','value-tick-up','value-tick-down',
      'stable-value-box','live-dashboard-roll','live-roll-whole-active'
    );
    element.classList.add('rolling-number','live-roll-number');
    element.style.removeProperty('--stable-value-width');
    element.style.removeProperty('inline-size');
    element.style.removeProperty('width');
  }

  function paintStatic(element,text){
    const fragment = document.createDocumentFragment();
    for(const character of String(text)) fragment.appendChild(staticCharacter(character));
    prepareElement(element);
    element.replaceChildren(fragment);
  }

  function saveState(element,text,value){
    element.dataset.liveRollText = text;
    element.dataset.rollText = text;
    if(Number.isFinite(value)){
      const stored = String(value);
      element.dataset.liveRollValue = stored;
      element.dataset.rollValue = stored;
    }
  }

  function animateWholeValue(element,previousText,nextText,direction,token){
    const stack = document.createElement('span');
    stack.className = `live-roll-whole live-roll-whole-${direction}`;

    const oldNode = document.createElement('span');
    oldNode.className = 'live-roll-whole-old';
    oldNode.textContent = previousText;

    const newNode = document.createElement('span');
    newNode.className = 'live-roll-whole-new';
    newNode.textContent = nextText;

    stack.append(oldNode,newNode);
    prepareElement(element);
    element.classList.add('live-roll-whole-active');
    element.replaceChildren(stack);

    element._liveRollTimer = setTimeout(() => {
      if(element.dataset.liveRollToken !== token) return;
      paintStatic(element,nextText);
    },ROLL_MS + 40);
  }

  rollValue = function(id,formatted,numericValue){
    if(!LIVE_IDS.has(id)){
      return inheritedRollValue.call(this,id,formatted,numericValue);
    }
    ...
    const previousText = String(
      element.dataset.liveRollText ??
      element.dataset.rollText ??
      element.textContent ?? ''
    );
    const previousValue = finite(element.dataset.liveRollValue ?? element.dataset.rollValue);
    const explicitValue = finite(numericValue);
    const nextValue = Number.isFinite(explicitValue)
      ? explicitValue
      : finite(typeof numericFromText === 'function' ? numericFromText(nextText) : NaN);
    ...
    if(previousText === nextText){
      saveState(element,nextText,nextValue);
      return;
    }

    clearTimeout(element._liveRollTimer);
    const token = String((Number(element.dataset.liveRollToken) || 0) + 1);
    element.dataset.liveRollToken = token;

    const comparable = previousText &&
    ...
    const direction = nextValue < previousValue ? 'down' : 'up';

    /* The dashboard uses a whole-value vertical transition whenever currency
       formatting changes the string length. This avoids mismatched digit slots. */
    if(previousText.length !== nextText.length){
      animateWholeValue(element,previousText,nextText,direction,token);
      return;
    }
    ...
    for(let index = 0; index < nextText.length; index++){
      ...
      const newCharacter = nextText[index];
      const changedDigit = /\d/.test(oldCharacter) &&
        /\d/.test(newCharacter) &&
        oldCharacter !== newCharacter;

      if(!changedDigit){
        fragment.appendChild(staticCharacter(newCharacter));
        continue;
      }

      changedDigits++;
      const slot = document.createElement('span');
      slot.className = `live-roll-slot roll-slot roll-${direction}`;

      const oldNode = document.createElement('span');
      oldNode.className = 'live-roll-old roll-old';
      oldNode.textContent = oldCharacter;

      const newNode = document.createElement('span');
      newNode.className = 'live-roll-new roll-new';
      newNode.textContent = newCharacter;
  ...
  function syncLiveButton(){
    const panel = document.getElementById('landscapeLive');
    const button = document.getElementById('drawerLandscapeLive');
    if(!panel || !button) return;
    const open = !panel.hidden && panel.getAttribute('aria-hidden') !== 'true';
    button.classList.toggle('active',open);
    button.setAttribute('aria-pressed',open ? 'true' : 'false');
    button.title = open
      ? 'Live Display attivo — premi per riaprirlo'
      : 'Visualizzazione LIVE a schermo intero su desktop e smartphone';
  }

  function initialise(){
    LIVE_IDS.forEach(id => document.getElementById(id)?.classList.add('live-roll-value'));
    const panel = document.getElementById('landscapeLive');
    if(panel){
      new MutationObserver(syncLiveButton).observe(panel,{
        attributes:true,
...
/* v10.5.0 — Premium visual system: accent presets, loading skeletons and chart pulse */
(() => {
  const STORAGE_KEY='inj_accent_v105';
  const PRESETS={
    green:{hex:'#22d3a6',rgb:'34,211,166',ink:'#03130f',label:'Injective Green'},
    blue:{hex:'#4d8dff',rgb:'77,141,255',ink:'#020817',label:'Electric Blue'},
    purple:{hex:'#a879ff',rgb:'168,121,255',ink:'#0a0414',label:'Neon Purple'},
    gold:{hex:'#f4c15d',rgb:'244,193,93',ink:'#171004',label:'Premium Gold'},
    mono:{hex:'#f3f5f7',rgb:'243,245,247',ink:'#050505',label:'Monochrome White'}
  };
  ...
  function applyAccent(name,{feedback=false}={}){
    if(!PRESETS[name]) name='green';
    ...
    document.querySelectorAll('[data-accent]').forEach(button=>{
      const active=button.dataset.accent===name;
      button.classList.toggle('active',active);
      button.setAttribute('aria-pressed',active?'true':'false');
    });

    const meta=document.querySelector('meta[name="theme-color"]');
    const theme=document.body?.dataset.theme||document.documentElement.dataset.theme||'black';
    if(meta && theme==='black') meta.setAttribute('content','#000000');

    requestAnimationFrame(()=>{
      try{ if(typeof drawAll==='function') drawAll(); }catch{}
      window.dispatchEvent(new Event('resize'));
    });
    if(feedback && typeof toast==='function') toast(`Accent: ${preset.label}`);
  }

  function initAccentPicker(){
    const selected=safeGet(STORAGE_KEY,document.documentElement.dataset.accent||'green');
    applyAccent(selected);
    document.getElementById('accentPicker')?.addEventListener('click',event=>{
      const button=event.target.closest('button[data-accent]');
      if(!button) return;
      applyAccent(button.dataset.accent,{feedback:true});
    });
  }
  ...
  function initSkeletons(){
    const ids=[
      'marketPrice','marketChange','netWorthUsd','netWorthInj',
      'availableInj','availableUsd','stakedInj','stakedUsd','rewardsInj','rewardsUsd',
      'aprValue','dailyEstimate','marketCapValue','commissionValue',
      'rewardPerSecond','oneInjEta','simulatedInjPrice','simCurrentMarketCap',
      'networkTotalStaked','networkInjBurned'
    ];
    const targets=ids.map(id=>document.getElementById(id)).filter(Boolean);
    targets.forEach(el=>el.classList.add('premium-skeleton-target'));
    document.body.classList.add('premium-hydrating');

    let released=false;
    const release=()=>{
      if(released) return;
      released=true;
      document.body.classList.remove('premium-hydrating');
      targets.forEach(el=>el.classList.remove('premium-skeleton-target'));
    };
    ...
    if(!document.body.classList.contains('app-loading')) setTimeout(release,500);
    setTimeout(()=>{observer.disconnect();release()},3600);
  }

  function initChartPulse(){
    window.setInterval(()=>{
      if(document.hidden) return;
      if(document.body.classList.contains('charts-hidden')) return;
      if(document.body.classList.contains('motion-off') || document.body.classList.contains('reduce-motion')) return;
      try{ if(typeof drawAll==='function') drawAll(); }catch{}
    },320);
  }
