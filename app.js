...
function drawChart(canvas, values, positive=true, hover=null){
  ...
  if(points.length===1){
    ...
    ctx.beginPath();ctx.moveTo(pad,h);ctx.lineTo(pad,p.y);ctx.lineTo(w-pad,p.y);ctx.lineTo(w-pad,h);ctx.closePath();ctx.fillStyle=gradient;ctx.fill();
    ctx.beginPath();ctx.moveTo(pad,p.y);ctx.lineTo(w-pad,p.y);ctx.strokeStyle=accent;ctx.lineWidth=2;ctx.lineCap='round';ctx.stroke();
    ctx.beginPath();ctx.arc(p.x,p.y,3.5,0,Math.PI*2);ctx.fillStyle=accent;ctx.fill();
  }
...
/* v10.1 — Live Display uses the dashboard per-digit rolling engine */
(() => {
  const LIVE_IDS = new Set([
    'landscapeMarketPrice','landscapeWalletWorth','landscapeWalletInj',
    'landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards'
  ]);
  const ANIMATION_MS = 590;
  const inheritedRollValue = rollValue;
  ...
  function paintStatic(element, text){
    ...
    element.classList.add('rolling-number','live-dashboard-roll');
  }
  ...
  rollValue = function(id, formatted, numericValue){
    ...
    const previousText = String(
      element.dataset.liveDashboardText ??
      element.dataset.liveAnimatedText ??
      element.dataset.rollText ??
      element.textContent ?? ''
    );
    const previousValue = finite(
      element.dataset.liveDashboardValue ??
      element.dataset.liveAnimatedValue ??
      element.dataset.rollValue
    );
    const nextValue = finite(numericValue);

    element.dataset.liveDashboardText = nextText;
    element.dataset.liveAnimatedText = nextText;
    element.dataset.rollText = nextText;
    if(Number.isFinite(nextValue)){
      element.dataset.liveDashboardValue = String(nextValue);
      element.dataset.liveAnimatedValue = String(nextValue);
      element.dataset.rollValue = String(nextValue);
    }

    if(previousText === nextText) return;

    const canRoll = motionEnabled() &&
    ...
    for(let index=0; index<nextText.length; index++){
      ...
      const changedDigit = /\d/.test(oldCharacter) && /\d/.test(newCharacter) && oldCharacter !== newCharacter;

      if(changedDigit){
        changedDigits++;
        const slot = document.createElement('span');
        slot.className = `roll-slot live-roll-slot roll-${direction}`;
    ...
    clearTimeout(element._liveDashboardRollTimer);
    const token = String((Number(element.dataset.liveDashboardToken) || 0) + 1);
    element.dataset.liveDashboardToken = token;
    element._liveDashboardRollTimer = setTimeout(() => {
      if(element.dataset.liveDashboardToken !== token) return;
      paintStatic(element,nextText);
    },ANIMATION_MS + 40);
  };
})();
...
/* v10.2 — continuous-track Live Display rolling, preserving the v10.1 layout */
(() => {
  const LIVE_IDS_V102 = new Set([
    'landscapeMarketPrice','landscapeWalletWorth','landscapeWalletInj',
    'landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards'
  ]);
  const FLUID_ANIMATION_MS = 740;
  const previousRollValueV102 = rollValue;

  const finiteV102 = value => {
    const number = Number(value);
    return Number.isFinite(number) ? number : NaN;
  };

  const motionEnabledV102 = () => !document.body.classList.contains('motion-off') &&
    !window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  function staticCharacterV102(character){
    const node = document.createElement('span');
    node.className = /\d/.test(character) ? 'roll-char roll-digit' : 'roll-char roll-symbol';
    node.textContent = character;
    return node;
  }

  function paintStaticV102(element,text){
    const fragment = document.createDocumentFragment();
    for(const character of String(text)) fragment.appendChild(staticCharacterV102(character));
    element.replaceChildren(fragment);
    element.classList.add('rolling-number','live-dashboard-roll');
  }

  rollValue = function(id,formatted,numericValue){
    if(!LIVE_IDS_V102.has(id)) return previousRollValueV102.call(this,id,formatted,numericValue);
    ...
    const nextText = String(formatted ?? '');
    const previousText = String(
      element.dataset.liveFluidText ??
      element.dataset.liveDashboardText ??
      element.dataset.liveAnimatedText ??
      element.dataset.rollText ??
      element.textContent ?? ''
    );
    const previousValue = finiteV102(
      element.dataset.liveFluidValue ??
      element.dataset.liveDashboardValue ??
      element.dataset.liveAnimatedValue ??
      element.dataset.rollValue
    );
    const nextValue = finiteV102(numericValue);

    element.dataset.liveFluidText = nextText;
    element.dataset.liveDashboardText = nextText;
    element.dataset.liveAnimatedText = nextText;
    element.dataset.rollText = nextText;
    if(Number.isFinite(nextValue)){
      const stored = String(nextValue);
      element.dataset.liveFluidValue = stored;
      element.dataset.liveDashboardValue = stored;
      element.dataset.liveAnimatedValue = stored;
      element.dataset.rollValue = stored;
    }

    if(previousText === nextText) return;

    const canRoll = motionEnabledV102() &&
      previousText.length === nextText.length &&
      Number.isFinite(previousValue) &&
      Number.isFinite(nextValue) &&
      previousValue !== nextValue;
    ...
    if(!canRoll){
      paintStaticV102(element,nextText);
      return;
    }

    const direction = nextValue > previousValue ? 'up' : 'down';
    const fragment = document.createDocumentFragment();
    const baseColor = getComputedStyle(element).color || 'currentColor';
    let changedDigits = 0;
    ...
    for(let index=0; index<nextText.length; index++){
      ...
      if(!changedDigit){
        fragment.appendChild(staticCharacterV102(newCharacter));
        continue;
      }

      changedDigits++;
      const slot = document.createElement('span');
      slot.className = `roll-slot live-roll-slot smooth-roll-slot roll-${direction}`;
      slot.style.setProperty('--live-roll-base',baseColor);

      const track = document.createElement('span');
      track.className = 'live-roll-track';

      const oldNode = document.createElement('span');
      oldNode.className = 'roll-old live-roll-old';
      oldNode.textContent = oldCharacter;

      const newNode = document.createElement('span');
      newNode.className = 'roll-new live-roll-new';
      newNode.textContent = newCharacter;

      if(direction === 'up') track.append(oldNode,newNode);
      else track.append(newNode,oldNode);
    ...
    clearTimeout(element._liveFluidRollTimer);
    const token = String((Number(element.dataset.liveFluidToken) || 0) + 1);
    element.dataset.liveFluidToken = token;
    element._liveFluidRollTimer = setTimeout(() => {
      if(element.dataset.liveFluidToken !== token) return;
      paintStaticV102(element,nextText);
    },FLUID_ANIMATION_MS + 30);
  };
})();
